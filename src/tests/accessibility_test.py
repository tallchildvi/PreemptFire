...
def test_dijkstra_30x30():
    ...
    passable_mask = np.ones((size, size), dtype=np.uint8)
    passable_mask[15, :] = 0
    ...
    assert np.all(np.isinf(optimal_time[15, :]))  # вода залишається inf
    ...
    finite_mask = np.isfinite(optimal_time)
    print("\nStatistics:")
    print(f"min:  {optimal_time[finite_mask].min():.6f} h")
    print(f"max:  {optimal_time[finite_mask].max():.6f} h")
    print(f"mean: {optimal_time[finite_mask].mean():.6f} h")
