...
def dijkstra_kernel(elevation: np.ndarray, sources: np.ndarray, resolution: float):
    if elevation.shape != sources.shape:
        raise ValueError(f"Shape mismatch: elevation shape {elevation.shape} must match sources shape {sources.shape}.")

    rows, cols  = elevation.shape
    optimal_time = np.where(sources == 1, 0, np.inf).astype(np.float32)
    queue = PriorityQueue()
